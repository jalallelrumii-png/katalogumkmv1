...
// ============================================================
// ⚙️  GANTI SATU BARIS INI DENGAN URL APPS SCRIPT LO
// Cara dapet URL: Apps Script → Deploy → New Deployment → Web App → Copy URL
// ============================================================
...
// ============================================================
// API — otomatis konek ke Google Sheet lo
// ============================================================
...
// Compress gambar sebelum upload — supaya cepat & tidak overload Apps Script
...
// ============================================================
// DATA DUMMY — dipakai saat APPS_SCRIPT_URL belum diisi
// Hapus / ignore setelah connect ke Google Sheet
// ============================================================
const DUMMY_SELLER = {
  id: "dummy",
  name: "Dapur Bu Sari",
  tagline: "Masakan rumahan enak & terjangkau 🍳",
  wa: "6281234567890",
  slug: "dapur-bu-sari",
  payment_mode: "both",
  logo: "🍳",
...
// Render gambar produk — bisa URL Drive, base64, atau emoji fallback
function ProductImage({ src, alt = "", className = "", style = {} }) {
  // Cek apakah src adalah URL atau base64 (bukan emoji)
...
// Helper ambil URL gambar dari produk (support field image atau image_url)
function getProductImage(p) {
  return p.image_url || p.image || "📦";
}
function isConnected() {
  return APPS_SCRIPT_URL !== "GANTI_URL_APPS_SCRIPT_DI_SINI";
}
...
// ============================================================
function SellerDashboard({ seller: initSeller, products: initProducts, onPreview }) {
  ...
  async function handleToggle(productId) {
    setProducts((ps) => ps.map((p) => String(p.id) === String(productId) ? { ...p, active: !p.active } : p));
  ...
  async function handleAddProduct() {
    ...
    setSaving(true);
    try {
      // Compress gambar kalau ada, sebelum kirim ke Apps Script
      let image_base64 = "";
      ...
      const payload = {
        ...
        seller_id: seller.id,
        active: true,
        image_base64, // Apps Script akan upload ini ke Drive
      };

      let finalImageUrl = image_base64 || "📦"; // fallback emoji kalau tidak ada gambar

      if (isConnected()) {
        const res = await API.saveProduct(payload);
        // Apps Script return image_url (URL Google Drive)
      ...
      const newEntry = { ...payload, image: finalImageUrl, image_url: finalImageUrl };
      setProducts((ps) => [...ps, newEntry]);
  ...
  function handleCopyLink() {
    navigator.clipboard?.writeText(`katalog.id/${seller.slug}`);
    setCopied(true);
  ...
  const statsData = [
    ...
    { label: "Kunjungan", value: "128", icon: <Users size={16} />, color: "#3b82f6" },
    { label: "Order Masuk", value: "23", icon: <ShoppingBag size={16} />, color: "#f59e0b" },
    { label: "Omzet Hari Ini", value: "Rp 1,2jt", icon: <TrendingUp size={16} />, color: "#8b5cf6" },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-white border-b px-4 py-4 flex items-center justify-between">
        ...
        <button onClick={onPreview} className="flex items-center gap-1 text-sm font-semibold px-3 py-2 rounded-xl" style={{ backgroundColor: color + "15", color }}>
          <Eye size={16} /> Preview
        </button>
      </div>
        ...
        <div className="mx-4 mt-4 space-y-3">
          ...
          {!isConnected() && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-4 text-sm text-yellow-700">
              ⚠️ Mode demo — belum connect ke Google Sheet. Ganti APPS_SCRIPT_URL di baris paling atas file ini.
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex flex-col justify-end">
          <div className="bg-white rounded-t-3xl p-4 space-y-3">
            ...
            <div>
              ...
              {imagePreview && (
                <button
                  onClick={() => { setImagePreview(null); setNewProduct(p => ({ ...p, image: "" })); }}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t flex">
        <button className="flex-1 py-4 flex flex-col items-center gap-1" style={{ color }}><Package size={20} /><span className="text-xs font-medium">Produk</span></button>
        <button className="flex-1 py-4 flex flex-col items-center gap-1 text-gray-300"><TrendingUp size={20} /><span className="text-xs">Analitik</span></button>
        <button className="flex-1 py-4 flex flex-col items-center gap-1 text-gray-300"><Settings size={20} /><span className="text-xs">Setting</span></button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  // Kalau URL mengandung slug (misal: katalog.id/dapur-bu-sari)
  // otomatis load katalog buyer tanpa harus ke landing
  useEffect(() => {
    const slug = window.location.pathname.replace("/", "").trim();
    if (slug && slug !== "" && isConnected()) {
      setLoading(true);
  ...
  if (view === "buyer-live" && catalogData) {
    return <BuyerView seller={catalogData.seller} products={catalogData.products} />;
  }
  if (view === "buyer") {
    return <BuyerView seller={DUMMY_SELLER} products={DUMMY_PRODUCTS} onBack={() => setView("landing")} />;
  }
  if (view === "seller") {
    return <SellerDashboard seller={DUMMY_SELLER} products={DUMMY_PRODUCTS} onPreview={() => setView("buyer")} />;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6">
      ...
      {!isConnected() && (
        <div className="text-xs text-yellow-600 bg-yellow-50 border border-yellow-200 px-3 py-2 rounded-xl mb-6 text-center">
          ⚠️ Mode demo — ganti APPS_SCRIPT_URL untuk connect ke Google Sheet
        </div>
      )}
      <div className="w-full max-w-sm space-y-3">
        <button onClick={() => setView("seller")} className="w-full py-4 rounded-2xl text-white font-bold text-lg shadow-lg" style={{ backgroundColor: "#16a34a" }}>
          🏪 Dashboard Seller
        </button>
        <button onClick={() => setView("buyer")} className="w-full py-4 rounded-2xl font-bold text-lg border-2" style={{ borderColor: "#16a34a", color: "#16a34a" }}>
          👀 Lihat Katalog (Demo)
        </button>
      </div>
    </div>
